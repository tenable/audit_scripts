...
def parse_args(parameters):
    ...
    parser.add_argument('-c', '--checks', nargs=1, default=None,
                        help='directory of audit checks')
    parser.add_argument('-o', '--output', nargs=1, default=default_audit,
                        help='output audit name')
    ...
    args.checks = list_or_string(args.checks)
...
def convert_script_to_item(source, encode=False, destination=None):
    global setting_re
...
'''
    ...
    # save check as individual file if requested
...
if __name__ == '__main__':
    ...
    for script in scripts:
        item = convert_script_to_item(script, args.encode, args.checks)
        if item is not None:
            items.append(item)
